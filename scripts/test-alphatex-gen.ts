/**
 * Spike: Generate AlphaTex from parsed SNG data.
 * Run with: npx tsx scripts/test-alphatex-gen.ts
 */
import { PSARC } from "../src/lib/rocksmith/psarcParser";
import { parseSNG, SongData, Note, Beat } from "../src/lib/rocksmith/sngParser";
...
function quantizeDuration(durationSec: number, bpm: number): number {
  ...
  // Map to standard durations: 1=whole, 2=half, 4=quarter, 8=eighth, 16=sixteenth, 32=thirty-second
...
/** Generate note effects string */
function getNoteEffects(note: Note, song: SongData): string {
  const effects: string[] = [];

  if (note.slideTo > 0) effects.push("s");
  if (note.vibrato > 0) effects.push("v");
  if (note.tap > 0) effects.push("tp");
  if (note.slap > 0) effects.push("ds"); // dead slap
  if (note.maxBend > 0) {
    // Bend values in Rocksmith: 1.0 = full step (4 quarter tones)
    const bendQuarters = Math.round(note.maxBend * 4);
    effects.push(`b (0 ${bendQuarters} ${bendQuarters} 0)`);
  }
  // Palm mute: check mask bit (bit 9 = 0x200)
  if (note.mask & 0x200) effects.push("pm");
  // Hammer-on: mask bit 1 = 0x2
...
function generateAlphaTex(song: SongData, arrangementName: string, songTitle: string, artistName: string): string {
  ...
  const bpm = secondBeat ? Math.round(60 / (secondBeat.time - firstBeat.time)) : 120;
  ...
  const musicalMeasures: { startTime: number; endTime: number; beats: Beat[] }[] = [];
  for (let i = 0; i < song.beats.length; i += BEATS_PER_BAR) {
    const barBeats = song.beats.slice(i, i + BEATS_PER_BAR);
    const endTime = (i + BEATS_PER_BAR < song.beats.length)
  ...
  let noteIdx = 0;
  const maxMeasures = 40; // Limit for spike

  for (let mi = 0; mi < Math.min(musicalMeasures.length, maxMeasures); mi++) {
    ...
    for (let i = 0; i < beatGroups.length; i++) {
      ...
      const formatNote = (n: Note): string => {
        if (n.chordId >= 0 && song.chordTemplates[n.chordId]) {
          const ct = song.chordTemplates[n.chordId];
          const chordNotes: string[] = [];
          for (let s = 0; s < 6; s++) {
            if (ct.frets[s] !== 255) {
              chordNotes.push(`${ct.frets[s]}.${6 - s}`);
            }
          }
        ...
        return `${n.fret}.${6 - n.string}`;
      };

      const effects = getNoteEffects(group[0], song);

      if (group.length === 1) {
        beatStrings.push(`${formatNote(group[0])}.${alphaDuration}${effects}`);
      } else {
        const chordNotes = group.map(n => `${n.fret}.${6 - n.string}`);
        beatStrings.push(`(${chordNotes.join(" ")}).${alphaDuration}${effects}`);
      }
    }
...
async function main() {
  console.log("=== AlphaTex Generation Spike ===\n");
  ...
  const songTitle = firstArr?.songName ?? "Unknown";
  const artistName = firstArr?.artistName ?? "Unknown";
  ...
  console.log(`SNG: ${files[leadIdx]}\n`);

  const sngData = await psarc.readFile(leadIdx);
  if (!sngData) { console.log("Failed to read SNG"); return; }

  const song = parseSNG(sngData);
  const alphaTex = generateAlphaTex(song, "Lead", songTitle, artistName);

  console.log("--- Generated AlphaTex (first 20 measures) ---\n");
  console.log(alphaTex);

  // Save to file for testing in browser
  const outPath = path.join(__dirname, "..", "test-output.alphatex");
  fs.writeFileSync(outPath, alphaTex);
  console.log(`\nSaved to: ${outPath}`);
}
